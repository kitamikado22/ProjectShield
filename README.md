...
## 目的
第一に、昔から考えていたパリィをメインにしたアクションゲームが作りたかったからです。

第二に、ゲーム会社のインターンシップで今までの自分自身のコーディングと実務レベルでのコーディングの差を痛感したため、少しでも学んだことを活かして**自分なりに拡張性・保守性を高めた設計を実際に行いたかったからです**。
前回の[MANFALL](https://github.com/kitamikado22/manfall/tree/portfolio)はとにかく一つのゲームを完成させることを目的としたため完成しましたが、今回のプロジェクトの目的は技術力を試すことであるため現時点で完成していないことに注意してほしいです。
